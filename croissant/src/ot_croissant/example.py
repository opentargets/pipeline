"""Example usage of the mlcroissant library (GPT-3 dataset)."""

import mlcroissant as mlc


def get_metadata():
    """Return example Croissant metadata for the GPT-3 dataset."""
        ...
        mlc.RecordSet(
            ...
            fields=[
                ...
                mlc.Field(
                    ...
                    source=mlc.Source(
                        file_set='jsonl-files',
                        extract=mlc.Extract(file_property=mlc._src.structure_graph.nodes.source.FileProperty.filename),
                        # Extract the field from a regex on the filename:
                        transforms=[mlc.Transform(regex=r'^(.*)\.jsonl$')],
                    ),
                ),
            ],
        )
