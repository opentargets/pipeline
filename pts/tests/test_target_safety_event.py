...
def test_process_toxcast_maps_columns(spark):
    ...
    row = Row(
        ...
        assay_component_desc='some assay',
...
def test_build_target_safety_events_output_columns(spark):
    ...
    safety = spark.createDataFrame([_safety_row(ensg='ENSG00000001')], SAFETY_SCHEMA)
    target = spark.createDataFrame([Row(id='ENSG00000001', approvedSymbol='G1', proteinIds=[])], TARGET_SCHEMA)
    diseases = spark.createDataFrame([Row(id='EFO_9999', obsoleteTerms=[])], DISEASE_SCHEMA)
