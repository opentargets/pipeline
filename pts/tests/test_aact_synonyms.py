...
LABEL_SOURCE_SCHEMA_T = ArrayType(
    StructType([
...
class TestChemblIndexes:
    def _mol_df(self, spark):
        ...
        data = [
            ...
            Row(
                id='CHEMBL9',
                name='Aspirin component of FOLFOX',
...
class TestAnchorCandidates:
    def test_synonym_anchors_novel_candidate(self, spark):
        ...
        name_index = spark.createDataFrame(
            ...
            StructType([
                StructField('name_norm', StringType()),
        ...
        pc = spark.createDataFrame(
            ...
            StructType([
                StructField('id', StringType()),
                StructField('related', ArrayType(StringType())),
            ]),
        )
    ...
    def test_conflict_status(self, spark):
        ...
        # entry anchors CHEMBL1 (via 'filgrastim'); 'aspirin' resolves to
        # unrelated CHEMBL5 -> CONFLICT for CHEMBL1
        entries = spark.createDataFrame(
        ...
        name_index = spark.createDataFrame(
            [
                Row(name_norm='filgrastim', ids=['CHEMBL1']),
                Row(name_norm='aspirin', ids=['CHEMBL5']),
            ],
            StructType([
                StructField('name_norm', StringType()),
                StructField('ids', ArrayType(StringType())),
    ...
    def test_parent_child_status(self, spark):
        ...
        # entry anchors CHEMBL1; 'pegfilgrastim' resolves to CHEMBL2 which is a
        # child of CHEMBL1 -> PARENT_CHILD
        entries = spark.createDataFrame(
        ...
        name_index = spark.createDataFrame(
            [
                Row(name_norm='filgrastim', ids=['CHEMBL1']),
                Row(name_norm='pegfilgrastim', ids=['CHEMBL2']),
            ],
            StructType([
                StructField('name_norm', StringType()),
                StructField('ids', ArrayType(StringType())),
    ...
    def test_exactly_cap_is_allowed(self, spark):
        ...
        # 10 == cap -> entry NOT poisoned; 'g-csf' (unresolved) is a NOVEL
        # candidate for each of the 10
        out = _anchor_candidates(entries, name_index, pc).collect()
        assert out != []
...
class TestCleanupRules:
    ...
    def test_drops_parent_child_and_noise(self, spark):
        ...
        regimen = spark.createDataFrame(
            ...
            StructType([
                StructField('regimen_norm', StringType()),
                StructField('ids', ArrayType(StringType())),
            ]),
        )
        existing = spark.createDataFrame(
            ...
            StructType([
                StructField('id', StringType()),
                StructField('existing', ArrayType(StringType())),
            ]),
        )
        rows = [
            {
    ...
    def test_word_boundary_not_substring(self, spark):
        ...
        # 'nystatin' contains 'statin' and 'cellcept' contains 'cell' as
        # SUBSTRINGS, not whole words -> kept
        rows = [
            {
                'id': 'CHEMBL1',
                'candidate': 'nystatin',
...
class TestRewriteAndReclassify:
    ...
    def test_rewritten_code_already_on_anchor_dropped(self, spark):
        # the extracted code is already a label of the anchor
        # CHEMBL1 -> redundant -> dropped
        out = self._run(
            spark,
            'mek inhibitor pd0325901',
            [{'name_norm': 'pd0325901', 'ids': ['CHEMBL1']}],
            [],
        )
        assert out == set()

    def test_rewritten_code_on_parent_child_reclassified(self, spark):
        # the extracted code resolves to CHEMBL2, a child of the anchor
    ...
    def test_rewritten_code_unrelated_is_conflict(self, spark):
        # the extracted code resolves to unrelated CHEMBL9 -> CONFLICT
        # (kept, per design)
        out = self._run(
            spark,
            'mek inhibitor pd0325901',
            [{'name_norm': 'pd0325901', 'ids': ['CHEMBL9']}],
            [],
        )
        assert out == {('pd0325901', 'CONFLICT')}

    def test_non_descriptor_candidate_passes_through(self, spark):
        # 'g-csf' has no class keyword and no extractable code -> unchanged
        # stays NOVEL
...
class TestMineAactSynonyms:
    ...
    def test_same_trial_duplicate_counts_once(self, spark):
        ...
        assert (
            'CHEMBL1',
            'g-csf',
        ) not in out  # only 1 distinct trial -> below MIN_TRIALS
