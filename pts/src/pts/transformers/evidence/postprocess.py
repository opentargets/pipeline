...
It reads nothing and writes nothing. `run` takes a `LazyFrame` and returns `LazyFrame`s, so the
same recipe serves both the shape we have and the shape we are moving to:

* today, one `evidence_postprocess_*` step per datasource reads a datasource's evidence, runs this,
  and writes the two outputs (`pts.transformers.evidence_postprocess`);
* later, a per-datasource module under this package GENERATES its evidence -- in polars where the
  port allows, otherwise in spark, landing parquet this reads back -- and runs the same recipe on
  the frame before writing. No second implementation, and no I/O assumptions to unpick.

Deliberately does NOT import `EXPRESSIONS`. Expressions arrive as a parameter, so the registry can
dissolve into the per-datasource modules that own them without this file changing at all. Whoever
constructs the postprocessor decides where its expressions come from -- today a registry lookup in
the step, later a module's own definition.
"""
...
class ValidationLuts:
    ...
    Built once from storage and shared across datasources, so they are passed in already
    materialised rather than being rebuilt per run. `pl.DataFrame`, not `LazyFrame`: these are
    joined against repeatedly and are small enough to hold.
    """
...
class PostprocessedEvidence:
    ...
    Kept lazy and unsplit until the caller asks: both sides recompute the upstream chain, and
    whether that cost is paid once or twice is the caller's decision, not this module's.
    """
...
class EvidencePostprocessor:
    ...
    Args:
        datasource_id: the `datasourceId` this evidence must carry; rows disagreeing are flagged.
        ...
        excluded_biotypes: target biotypes to reject, for the datasources that restrict them.
    """
    ...
    def run(self, lf: pl.LazyFrame, luts: ValidationLuts) -> PostprocessedEvidence:
        ...
        The call order is load-bearing and matches the pyspark implementation this replaced: later
        steps read columns earlier ones add, and `validate_uniqueness` in particular depends on the
        `id` that `assign_evidence_identifier` derives.
