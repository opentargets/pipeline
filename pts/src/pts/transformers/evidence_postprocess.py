...
`pts.pyspark.evidence_postprocess`. This module is deliberately thin: it turns otter's
`source`/`destination`/`settings` into the parameters the recipe needs, reads, runs
`pts.transformers.evidence.postprocess`, and writes. All the actual processing lives in the
`pts.transformers.evidence` package.
...
from pts.transformers.evidence.read import read_evidence
...
def evidence_postprocess(
    ...
    processed = postprocessor.run(read_evidence(source['evidence_path'], settings['evidence_format']), luts)
